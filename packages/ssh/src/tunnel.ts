...
export interface RemoteT3RunnerOptions {
  /** Operator-provided npm tarball; takes precedence over the release archive. */
...
function requiresRemoteNode(runner: RemoteT3RunnerOptions | undefined): boolean {
  return Boolean(runner?.nodeScriptPath?.trim() || runner?.packageSpec?.trim());
}
...
function sshRunnerLogFields(runner: RemoteT3RunnerOptions | undefined) {
  ...
  if (runner?.packageSpec?.trim()) {
    return { runner: "package-spec", packageSpec: runner.packageSpec.trim() };
  }
...
const REMOTE_PACKAGE_SPEC_SCRIPT = `# npm extracts a package before it runs the native builds of its dependencies,
...
run_package_spec() {
  ...
  # launcher attempt then stacks behind it (fork/README.md: SSH launch runs
...
export class SshMissingRunnerError extends Schema.TaggedError<SshMissingRunnerError>()(
  ...
  override get message(): string {
    return "A remote t3 runner needs an archive version, a node script path, or an explicit package spec.";
  }
}
...
export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  ...
  if (nodeScriptPath === "" && archiveVersion === "" && packageSpec === "") {
  ...
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      T3_PACKAGE_SPEC_SCRIPT:
        packageSpec && !nodeScriptPath
          ? applyScriptPlaceholders(REMOTE_PACKAGE_SPEC_SCRIPT, {
...
export function buildRemoteLaunchScript(input?: RemoteT3RunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    T3_ARCHIVE_MODE: requiresRemoteNode(input) ? "0" : "1",
    T3_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  ...
  > {
    ...
    const result = yield* runSshCommand(target, {
      ...
      timeoutMs: requiresRemoteNode(runner)
...
> {
  ...
  const result = yield* runSshCommand(target, {
    ...
    ...(requiresRemoteNode(runner) ? {} : { timeoutMs: REMOTE_ARCHIVE_LAUNCH_TIMEOUT_MS }),
    ...(input?.authSecret === undefined ? {} : { authSecret: input.authSecret }),
    ...(input?.batchMode === undefined ? {} : { batchMode: input.batchMode }),
    ...(input?.interactiveAuth === undefined ? {} : { interactiveAuth: input.interactiveAuth }),
  });
