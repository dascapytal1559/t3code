...
Send steers the running turn. To wait until it finishes, use **Queue** or
`Cmd+Enter` on macOS / `Ctrl+Enter` on Windows and Linux. Mobile has a queue
button next to Send. Remove a queued message to cancel it.

The queue stays on this client and survives reloads. It sends the oldest message
when the thread is idle, while the app is open. On a new thread, the shortcut
still starts work in the background.
