...
describe("ClaudeAdapterLive", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (turnCompleted?.type === "turn.completed") {
        ...
        assert.deepEqual(turnCompleted.payload.resumeCursor, {
          ...
          turnStartMessageIds: [turn.turnId],
        });
      }
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
  ...
  it.effect.each(["result text", "assistant error"] as const)(
    "fork: recycles the session when %s reports a latched auth error",
    (source) => {
      const harness = makeHarness();
      return Effect.gen(function* () {
        const adapter = yield* ClaudeAdapter;
        ...
        if (source === "assistant error") harness.query.emit(AUTH_FAILURE_ASSISTANT);
        harness.query.emit({
          type: "result",
          subtype: "success",
          is_error: true,
          result: source === "result text" ? "Not logged in · Please run /login" : "",
          ...(source === "assistant error" ? { terminal_reason: "api_error" } : {}),
          num_turns: 1,
          session_id: "sdk-session-auth",
          uuid: "result-auth-1",
        } as unknown as SDKMessage);
        const events = Array.from(yield* Fiber.join(eventsFiber));
        const warningIndex = events.findIndex((event) => event.type === "runtime.warning");
        const completedIndex = events.findIndex((event) => event.type === "turn.completed");
        assert.isAtLeast(warningIndex, 0);
        assert.isAbove(completedIndex, warningIndex);
        assert.equal(String(events[warningIndex]?.turnId), String(turn.turnId));
        assert.equal(String(events[completedIndex]?.turnId), String(turn.turnId));
  ...
  it.effect("rewinds a steered Claude turn after recovery and preserves fork boundaries", () => {
    ...
    return Effect.gen(function* () {
      ...
      harness.query.emit({
        ...
        uuid: "assistant-1-final",
        parent_tool_use_id: null,
        message: { id: "assistant-message-1", content: [{ type: "text", text: "first reply" }] },
      } as unknown as SDKMessage);
      harness.query.emit({
        type: "result",
        subtype: "success",
      ...
      assert.deepEqual((yield* adapter.listSessions())[0]?.resumeCursor, {
        ...
        resumeSessionAt: "fork-assistant-1-final",
        turnAnchors: [{ turnId: firstTurnId, uuid: "fork-assistant-1-final" }],
      });
