...
export const make = Effect.gen(function* () {
      ...
      Effect.map(({ refinedProviders, projects }) => {
        ...
        for (const project of projects) {
          ...
          if (api !== null) {
            ...
            const root = projectVcsRoot(project);
            if (roots === undefined) viewerRoots.set(host, [root]);
  ...
  const viewerPermissionsOf = (
    ...
    project.api
      .getViewerPermissions({
        cwd: projectVcsRoot(project.project),
    ...
    Effect.forEach(
      ...
      (host) =>
        Effect.flatMap(Clock.currentTimeMillis, (now): Effect.Effect<ResolvedViewer> => {
          ...
          const roots =
            viewerRoots.get(host) ?? forHost.map(({ project }) => projectVcsRoot(project));
  ...
  const listUncached: PullRequestService["Service"]["list"] = (input) =>
    Effect.gen(function* () {
      ...
      const readTogether = (
        ...
        return readAcross({
          cwd: projectVcsRoot(first.project),
          host: first.host,
    ...
    Effect.gen(function* () {
      ...
      const result = yield* api
        .withVerifiedCredential(
          { cwd: projectVcsRoot(project.project), host: project.host },
          (identity) =>
    ...
    Effect.gen(function* () {
      ...
      const stats = yield* Effect.forEach(
        ...
        (entries) => {
          ...
          return readStats({
            cwd: projectVcsRoot(first.project.project),
  ...
  const persistedRead = Effect.fn("PullRequestService.persistedRead")(function* <A>(
    ...
    const key = [
      ...
      project.project.id,
      projectVcsRoot(project.project),
      String(input.number),
