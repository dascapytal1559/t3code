import { resolveProjectVcsRootInput } from "@t3tools/client-runtime/state/projects";
...
function RepositoryDirectoryRow({ member }: { member: SidebarProjectGroupMember }) {
  const primaryEnvironmentId = usePrimaryEnvironmentId();
  const updateProject = useAtomCommand(projectEnvironment.update);
  const vcsRoot = member.vcsRoot ?? null;
  const setVcsRoot = useCallback(
    async (value: string | null) => {
      if (value === vcsRoot) return;
      await updateProject({
        environmentId: member.environmentId,
        input: { projectId: member.id, vcsRoot: value },
      });
    },
    [member.environmentId, member.id, updateProject, vcsRoot],
  );
  const browse = useCallback(async () => {
    const picked = await readLocalApi()?.dialogs.pickFolder({
      initialPath: vcsRoot ?? member.workspaceRoot,
      targetEnvironmentId: member.environmentId,
    });
    if (picked) await setVcsRoot(picked);
  }, [member.environmentId, member.workspaceRoot, setVcsRoot, vcsRoot]);
  return (
    <SettingsRow
      title={member.environmentLabel ?? "Repository directory"}
      description={`Source control for ${member.workspaceRoot}. Relative paths resolve against this checkout's project root.`}
      status={vcsRoot ? "Overridden" : "Project root"}
      resetAction={
        vcsRoot ? (
          <SettingResetButton
      ...
      control={
        <div className="flex w-full items-center gap-2 sm:w-96">
          <Input
            key={`${member.physicalProjectKey}:${vcsRoot ?? ""}`}
            size="sm"
            className="w-full"
            aria-label={`Repository directory for ${member.workspaceRoot}`}
            placeholder={member.workspaceRoot}
            defaultValue={vcsRoot ?? ""}
            onBlur={(event) =>
              void setVcsRoot(
                resolveProjectVcsRootInput(event.currentTarget.value, member.workspaceRoot),
              )
            }
            onKeyDown={(event) => {
          ...
          />
          {isElectron && member.environmentId === primaryEnvironmentId ? (
  ...
  return (
    <>
      <SettingsPageContainer className="gap-6">
        ...
        <SettingsSection title="Repository directory">
          {group.memberProjects.map((member) => (
            <RepositoryDirectoryRow key={member.physicalProjectKey} member={member} />
          ))}
        </SettingsSection>
