...
const projectMocks = vi.hoisted(() => ({
  ...
  readFile: vi.fn(),
  // Fork: the entries query subscribes to watcher events and rescans on
  // manual refresh (FORK_FEATURES.md: Live filesystem updates).
  entriesEvents: vi.fn(),
...
const reactHooks = vi.hoisted(() => {
  ...
  return {
    ...
    useState<A>(initialValue: A): [A, (next: A) => void] {
      const index = nextIndex();
      refs[index] ??= { current: initialValue };
      const ref = refs[index] as { current: A };
      return [
        ref.current,
        (next) => {
          ref.current = next;
...
vi.mock("~/state/use-atom-command", () => ({
  useAtomCommand: () => () => Promise.resolve(),
}));
...
describe("project query refresh", () => {
  beforeEach(() => {
    ...
    projectMocks.entriesEvents.mockReset();
    projectMocks.entriesEvents.mockReturnValue(Atom.make(() => undefined));
    reactHooks.reset();
  });
