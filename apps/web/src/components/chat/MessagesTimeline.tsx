...
interface TimelineRowSharedState {
  ...
  onRevertToTurnCount: (targetTurnCount: number, restorePrompt?: string) => void;
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <div className="flex items-center gap-0.5">
            {typeof revertTurnCount === "number" && (
              <RevertUserMessageButton turnCount={revertTurnCount} prompt={row.message.text} />
            )}
...
function RevertUserMessageButton({ turnCount, prompt }: { turnCount: number; prompt: string }) {
  const ctx = use(TimelineRowCtx);
      ...
      <TooltipTrigger
        render={
          <Button
            ...
            onClick={() => ctx.onRevertToTurnCount(turnCount, prompt)}
...
function AssistantMessageMeta({
  ...
  return (
    ...
    >
      ...
      />
      {ctx.onForkFromTurn !== null && message.turnId !== null && !message.streaming && (
        <ForkFromTurnButton turnId={message.turnId} />
      )}
      {!message.streaming && (
        <Tooltip>
          <TooltipTrigger render={<p className="text-muted-foreground text-xs tabular-nums" />}>
