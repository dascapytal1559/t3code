...
export async function checkoutNewTaskBranch<E>(input: {
  ...
  readonly project: Pick<EnvironmentProject, "environmentId" | "workspaceRoot" | "vcsRoot"> | null;
  ...
  const result = await input.switchRef({
    ...
    // Checkouts run in the project's repository, not its workspace root.
    input: { cwd: projectVcsRoot(input.project), refName: input.branch.name },
  });
